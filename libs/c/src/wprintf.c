...
static const wchar_t *parse_format(const wchar_t *format_string, va_list *arg, print_format *format) {
    ...
    switch (c) {
        ...
        case L'e':
        case L'E':
            if (f.argument_options == short_argument || f.argument_options == char_argument ||
...
static wchar_t *double2hex(long double num, wchar_t *buff, print_format format) {
    ...
    for (i = 0; i < TARGET_FLOAT_BYTES; i++) {
        unsigned char x                  = nump[i];
        nump[i]                          = nump[TARGET_FLOAT_BYTES - i + 3];
        nump[TARGET_FLOAT_BYTES - i + 3] = x;
    }
    ...
    if (TARGET_FLOAT_IMPLICIT_J_BIT) {
        mantissa_bit = (4 + expbits + format.precision * 4) - 1;
    } else {
...
void round_decimal(decimal *dec, int new_length) {
    ...
    p = (unsigned char *) dec->sig.text + new_length + 1;
    c = *--p - L'0';

    if (c == 5) {
        unsigned char *q = &((unsigned char *) dec->sig.text)[dec->sig.length];
...
wchar_t *float2str(long double num, wchar_t *wbuff, print_format format) {
    ...
    form.digits = 0x20;
    __num2dec(&form, num, &dec);
    p = (unsigned char *) dec.sig.text + dec.sig.length;

    while (dec.sig.length > 1 && *--p == L'0') {
        --dec.sig.length;
        ++dec.exp;
    }
    ...
    switch (format.conversion_char) {
        ...
        e_format:
            ...
            *--p = format.conversion_char;
            ...
            for (n = dec.sig.length, q = (unsigned char *) dec.sig.text + dec.sig.length; --n;) {
                *--p = *--q;
            }
        ...
        f_format:
            ...
            q = (unsigned char *) dec.sig.text + dec.sig.length;
...
int __wpformatter(void *(*WriteProc)(void *, const wchar_t *, size_t), void *WriteProcArg, const wchar_t *format_str,
    ...
    while (*format_ptr) {
        ...
        if (format.justification_options != left_justification) {
            ...
            if (((*buff_ptr == L'+') || (*buff_ptr == L'-') || (*buff_ptr == L' ')) && (fill_char == '0')) {
